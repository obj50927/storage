...
# Preprocess the data 
# the data will have 97 attribute and binary values for each attribute
...
# get most common value of the class label in the subset
    ...
    if len(c_l) < 2:
        ...
        if is_limit != 0:
            if temp_depth == is_limit:
                ...
                return root
        # if subset empty, return leaf node
        # else call ID3 recursively
        if len(sub_set_true) < 2:
...
# map the model from root to predict the class label
...
# test the data and return the accuracy  
...
def prune(root, model, val_set, acc):
    ...
    if (root.true.value in class_label_set) and (root.false.value in class_label_set):
        # if a non-leaf node which branches has only class label set
        # prune
        ...
        # if pruned model have lower accuray then before,
        # return the node to original
        if temp_ac < acc:
            if is_true == 1:
                parent.true = root
            else:
...
def main():
    ...
    if mode == "vanilla":
        ...
        while (1):
            ...
            if new_acc > g_prune_acc:
                ...
                break
        val_set_accuracy = testing(val_set, model)
        #print("Val set accuracy: ", val_set_accuracy) 
    else:
        val_set = train_data[fifth + 1:]
        val_set.insert(0,train_data[0])
    ...
    #t_most_common = get_most_common(test_data, None)
    #print("Train_base: ", 1-baseline(train_data1, c_most_common))
    #print("Test_base: ",1-baseline(test_data, t_most_common))
